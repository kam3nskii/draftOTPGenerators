...
std::string getOTP(const std::vector<uint8_t>& bytes)
{
...
std::string generateOTP(const std::string& secretKey, const std::string& request)
{
    std::vector<uint8_t> bytes(BUFFER_SIZE);
    execute("./sha256.py", secretKey.c_str(), request.c_str());
...
{
    std::string secretKey{"1234567qwerty7654321"};
